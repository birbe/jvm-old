...
impl ControlFlow {
    ...
    fn mark_scopes(&mut self, graph: NodeGraph<IndexedBytecode>) -> Vec<ScopeType> {
        ...
        for index in 0..scoped_view.len() {
            ...
            node.front_edges.iter().for_each(|front_id| {
                if *front_id < index { //Loop, create a scope ending at this instruction and beginning at the target
                    ...
                    scoped.get_mut(*front_id).unwrap().2.push(BlockType::Loop(index-*front_id));
                    ...
                    scoped.get_mut(begin).unwrap().2.push(BlockType::Block(index-begin));
                }
            });
        }
